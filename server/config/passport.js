  ...
  passport.use(new GoogleStrategy({
    ...
    try {
      ...
      if (user) {
        ...
        if (googleName && (!user.name || user.name === 'Company Recruiter' || user.name === email.split('@')[0])) {
          user.name = googleName;
        }
        if (!user.password) {
          user.password = `google_auth_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
        }
        user.lastLogin = new Date();
        await user.save({ validateBeforeSave: false });
      } else {
        const autoGeneratedPassword = `google_auth_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
